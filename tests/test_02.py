...
CODE_FOLDER = [pathlib.Path("./expert_src/memory_agent")]
...
class BasicRequirements(BaseModel):
    presence_of_interrupt: bool # 1 points
    user_id_and_category_wise_storage: bool # 1 points
...
def test_best_practices_llm_judge():
    score = {"candidate": CANDIDATE_NAME, "bucket": "code_quality", "points": 0, "max_points": 21, "details": []}
    ...
    _add(score, 1 if judge.basic_requirements.presence_of_interrupt else 0, "presence_of_interrupt", judge.basic_requirements.presence_of_interrupt, "Presence of interrupt is present" if judge.basic_requirements.presence_of_interrupt else "Presence of interrupt is not present")
    _add(score, 1 if judge.basic_requirements.user_id_and_category_wise_storage else 0, "user_id_and_category_wise_storage", judge.basic_requirements.user_id_and_category_wise_storage, "User id and category wise storage is present" if judge.basic_requirements.user_id_and_category_wise_storage else "User id and category wise storage is not present")
